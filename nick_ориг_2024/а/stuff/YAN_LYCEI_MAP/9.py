...
class MapWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # туц-new
        self.new_code_box = QCheckBox(
            "Приписывать почтовый индекс", self
        )
        ...
        self.layout.addWidget(self.new_code_box)
        ...
        self.new_code_box.setChecked(True)  
    ...
    def update_map(self):
        try:
            ...
            QMessageBox.warning(self, "Ошибка", str(error_message))
    ...
    def search_object(self):
        try:
            ...
            # code_решение_новый
            include_postal_code = self.new_code_box.isChecked()
            if include_postal_code:
                address_with_postal_code = found_objects[0]["GeoObject"][
                    "metaDataProperty"
                ]["GeocoderMetaData"]["text"]
                #
                postal_code = found_objects[0]["GeoObject"]["metaDataProperty"][
                    "GeocoderMetaData"
                ]["Address"]["postal_code"]
                if postal_code:
                    address_with_postal_code += f", {postal_code}"
                self.address_display.setText(address_with_postal_code)
            else:
                self.address_display.setText(
                    found_objects[0]["GeoObject"]["metaDataProperty"][
                        "GeocoderMetaData"
                    ]["text"]
                )
        except ValueError as error_message:
