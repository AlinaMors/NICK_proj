...
from PyQt5.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QLabel, QLineEdit, QPushButton, QMessageBox, QRadioButton
)
...
class MapWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.load_button = QPushButton("показать Карту", self)
    ...
    def search_object(self):
        try:
            ...
            if not search_query:
                raise ValueError("Введите запрос(ай-йай)")

            response = requests.get(f"https://geocode-maps.yandex.ru/1.x/?apikey={self.map_manager.api_key}&format=json&geocode={search_query}")
            if response.status_code != 200:
                raise ValueError("упсссс")

            response_data = response.json()
            found_objects = response_data['response']['GeoObjectCollection']['featureMember']
            if not found_objects:
                raise ValueError("хватит выдумывать обекты")
            ...
            QMessageBox.warning(self, "сори нот сори", str(ve))
        except Exception as e:
            QMessageBox.warning(self, "Ошибка", "вообще не хочу хнать откуда эта ошибка взяласься")
