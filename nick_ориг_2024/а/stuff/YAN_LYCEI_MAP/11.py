...
class MapWidget(QWidget):
    ...
    def mouse_thing(self, event, search_function):
        ...
        lon = (click_position.x() / map_size.width()) * 360 - 180
        lat = 90 - (click_position.y() / map_size.height()) * 180
        self.clear_Results()
    ...
    def search_organization(self, lon, lat):
        ...
        if found_organizations:
            ...
            organization_lon, organization_lat = first_organization["geometry"][
                "coordinates"
            ]
            distance = self.distance_answer(
                lon, lat, organization_lon, organization_lat
            )
            if distance <= 50:
